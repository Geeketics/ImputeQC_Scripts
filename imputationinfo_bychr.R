...
##Make actual plots
for(i in unique(infoscore$CHR)){
  manhattanname = paste0(args[1], "_chr", i, "_manhattan", ".jpeg")
  jpeg(file = manhattanname, width = 1600, height = 800)
  manhattan.generic(x = infoscore[infoscore$CHR == i, ], chr = "CHR", bp = "POS", value = "INFO", ylabel = "Info Score", suggestiveline = 0.3, genomewideline = 0.8)
  dev.off()
  cat(paste(manhattanname, " file succesfully generated.\n", sep = ""))
}
