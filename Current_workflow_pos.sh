...
then
	...
	#filter samples missing overall >2% or SNPs missing >3%
	plink2 --bfile data/${infile} --make-bed --out temp/data --allow-no-sex

	#Split into chromosome files
	parallel "plink2 --bfile temp/data --make-bed --out temp/data{}_tmp --allow-no-sex --chr {}" ::: $(seq 1 22)

	#filter samples missing >5% per chromosome
	parallel "plink2 --bfile temp/data{}_tmp --make-bed --out temp/data{} --allow-no-sex --chr {} " ::: $(seq 1 22)
fi
...
#parallel 'tabix -p vcf vcf/data{}_strandaligned.samples_removed.vcf.gz' ::: $(seq 1 22)
...
then
	...
	plink2 --vcf vcf/all_chr_strandaligned.filt_refallele_maf0.001.vcf --recode vcf --out vcf/all_chr_strandaligned.filt_refallele_maf0.01_hwe_checked --a2-allele vcf/1kgAlleles.txt 3 2 --maf 0.01

else
	echo Removing HWE SNPs
	plink2 --vcf vcf/all_chr_strandaligned.filt_refallele_maf0.001.vcf --recode vcf --out vcf/all_chr_strandaligned.filt_refallele_maf0.01_hwe_checked --a2-allele vcf/1kgAlleles.txt 3 2 --exclude-snps $(cat vcf/hardy_exclude.txt) --maf 0.01
fi
